...
class AmazonBinImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_name = os.path.join(self.root_dir, "%05d.jpg" % img_id)
...
def test(model, test_loader, criterion, optimizer, epoch, device, hook):
    """
...
def net():
    ...
    model.fc = nn.Sequential(
        nn.Linear(2048, 128), nn.ReLU(inplace=True), nn.Linear(128, 6)
    )
    return model
...
def create_data_loaders(data, batch_size):
    ...
    train_data = AmazonBinImageDataset(
        csv_file=train_mdata_path, root_dir=data, transform=train_transform
    )
    ...
    test_data = AmazonBinImageDataset(
        csv_file=test_mdata_path, root_dir=data, transform=test_transform
    )
    ...
    validation_data = AmazonBinImageDataset(
        csv_file=validation_mdata_path, root_dir=data, transform=train_transform
    )
...
def main(args):
    ...
    for epoch in range(args.epochs):
        model = train(model, train_loader, criterion, optimizer, epoch, device, hook)

        test(model, test_loader, criterion, optimizer, epoch, device, hook)
