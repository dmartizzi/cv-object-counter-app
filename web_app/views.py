...
def input():
    ...
    form_opts_path = os.path.join(
        os.getcwd(), "web_app/static/textdata/form_options.csv"
    )
    form_opts = pd.read_csv(form_opts_path)
    form_opts = form_opts["img_name"].tolist()

    return render_template("input.html", form_opts=form_opts)
...
def output():
    ...
    return render_template("output.html", img_name=img_name, obj_count=obj_count)
