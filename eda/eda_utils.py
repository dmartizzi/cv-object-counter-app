...
def create_image_subsample(n_full=500000, n_sel=50000):
    """
...
def generate_image_ids_sel(
    image_ids_sel_path="image_ids_sel.csv", n_full=500000, n_sel=50000
):
    """
    ...
    image_ids_sel = pd.DataFrame(data={"id": create_image_subsample(n_full, n_sel)})
    image_ids_sel.sort_values(
        by=["id"], ascending=True, ignore_index=True, inplace=True
    )
...
def get_image_item_count(idd, s3bucket, s3prefix):
    """
...
def get_image(idd, s3bucket, s3prefix):
    """
...
def train_val_test_split(image_ids_sel, split=(0.8, 0.1, 0.1)):
    """
    ...
    full_df = (
        image_ids_sel.copy().sample(frac=1.0, random_state=1294).reset_index(drop=True)
    )
    ...
    if n_full > 0:
        if n_val_ini - n_train_ini > 0:
            train_df = full_df.iloc[0:n_val_ini, :].sort_values(
                by=["id"], ascending=True, ignore_index=True
            )

        if n_test_ini - n_val_ini > 0:
            val_df = full_df.iloc[n_val_ini:n_test_ini, :].sort_values(
                by=["id"], ascending=True, ignore_index=True
            )

        if n_full - n_test_ini > 0:
            test_df = full_df.iloc[n_test_ini:n_full, :].sort_values(
                by=["id"], ascending=True, ignore_index=True
            )
...
def transfer_images(idd, s3source, s3source_prefix, s3dest, s3dest_prefix):
    """
    ...
    key = "%05d.jpg" % idd
    copy_source = {"Bucket": s3source, "Key": os.path.join(s3source_prefix, key)}
    bucket = client.Bucket(s3dest)
    bucket.copy(copy_source, os.path.join(s3dest_prefix, key))


def transfer_metadata(idd, s3source, s3source_prefix, s3dest, s3dest_prefix):
    """
